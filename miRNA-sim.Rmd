...
* no mrNA synthesis (OK in oocytes, questionable in somatic cells). TODO: maybe consider some synthesis as well?
* only uncleaved mRNA bound in complex with AGO2 + miRNA will not be observed in experiments.
...
Where $k_{on}$,${k_{off}}$ and $k_{cat}$ are the reaction rates in the system. This is the same system as used in Michaelis-Menten kinetics, except we don't make the quasi steady state assumption. The assumption probably holds well in our case but we didn't check and stay on the safe side.
...
We test dynamics in two cell types - volume in picolitres, total_RNA in copies per cell (cpc):
```{r}
cells %>% mutate(volume_pl = volume * 10^12) %>% select(-volume) %>%  kable()
```
...
plot_concentration_surface <- function(to_show) {
 ...
 p <- to_show %>% filter(type == "target") %>% group_by(fraction_of_total_RNA, miRNA_count, name) %>% 
  ...
  ggplot(aes(x=time_h, y = k_cat_coeff, fill = relative_concentration, z = relative_concentration)) + 
   ...
   facet_grid(name~miRNA_count, labeller= labeller(miRNA_count = label_miRNA_count)) +
...
We see that all miRNA is expected to be degraded in 3T3 cells across a broad range of conditions (possibly accounting also for the case of non zero synthesis), while in oocytes we don't see noticeable degradation in most conditions.
