...
output:
  html_document:
    code_folding: hide
absract: "This is a supplement to the paper <TODO: paper title>. We describe a simplified mathematical model of the miRNA pathway based on our knowledge of the siRNA pathway."
---
...
* miRNA degradation is modelled as one step process (similar to siRNA/RNAi pathway), but with lower catalytic efficiency than the siRNA pathways
...
Where $k_{on}$,${k_{off}}$ and $k_{cat}$ are the reaction rates in the system and $s_{excess}$ is the excess synthesis rate of new mRNA (i.e. synthesis minus degradation by normal cell processes). This is the same system as used in Michaelis-Menten kinetics.
...
For the follwoing we use the siRNA $k_{cat}$ as reported. 
...
Since the paper primarily studies the let7 miRNA, we start with a simulation exploring the possible dynamics of a miRNA roughly similar to let7 under various conditions.

```{r}
log_scale = TRUE
N_steps = 10 #TODO: increase to 100 or more

normal_miRNA_cpc_3T3 = 38000
normal_miRNA_cpc_oocyte = 20000

min_miRNA_cpc_3T3 =  100
...
min_miRNA_concentration = (min_miRNA_cpc_3T3 / mole_unit) / volume_3T3
...
cell_info = tibble(condition = c("let7 in 3T3", "let7 in oocyte"),
                   miRNA_concentration = c(normal_miRNA_cpc_3T3 / (mole_unit * volume_3T3), normal_miRNA_cpc_oocyte / (mole_unit * volume_mouse_oocyte)))
```

Reformulate via Concentration?!

The let7 miRNA was found to have around `r normal_miRNA_cpc_3T3` copies per cell (cpc) in 3T3 somatic cells and around `r normal_miRNA_cpc_oocyte` cpc in oocytes. To be on the safe side, we explore range from `r min_miRNA_cpc_3T3` cpc to `r max_miRNA_cpc_3T3` cpc. 
We also do not know, how many let7 targets there are, so we explore range from very low target cpc (`r min_target_concentration`) to much larger number of targets than let7 (`r max_target_concentration`).
...
plot_efficiency_surface <- function(to_show, time_h, fixed_data) {
 ...
 p <- to_show %>% filter(type == "target", time_h == !!time_h) %>% 
  ...
  ggplot(aes(x = total_enzyme, y = total_target, fill = relative_concentration, z = relative_concentration)) + 
   ...
   geom_point(data = fixed_data, mapping = aes(x = miRNA_concentration, y = min_target_concentration / 5, shape = condition), inherit.aes = FALSE, size = 2) +
   geom_rect(data = fixed_data, mapping = aes(xmin = miRNA_concentration * 0.75, xmax = miRNA_concentration * 1.33, ymin = min_target_concentration / 3, ymax = max_target_concentration * 3), color = "lightgreen", fill = "transparent", inherit.aes = FALSE) +
   #scale_fill_distiller("Target uncleaved", palette = "Spectral") +
   scale_fill_gradient("Target uncleaved", low = "#505050", high = "#f0f0f0") +
   scale_linetype_manual(values = c("solid","dotted")) + 
   scale_shape_manual("Condition", values = c("triangle","triangle open")) +
   facet_wrap(~k_cat_coeff, labeller= labeller(k_cat_coeff = function(x) {paste0("Efficiency = ",x)})) +
...
* We bundle together all miRNA species and all target RNA molecules


```{r}
# cells <- tibble(
#   name = c("3T3","mouse oocyte", "pig oocyte"),
#   volume = c(volume_3T3,260e-12, 520e-12),
#   total_RNA = c(total_RNA_3T3, 25e6, 50e6),
#   excess_RNA_synthesis = c(0, 0, 0)
# ) %>% mutate(
...
#target_pools <- tibble(fraction_of_total_RNA = c(0.005,0.01,0.02, 0.04, 0.06, 0.1, 0.2))

#miRNA_counts <- tibble(miRNA_count = c(5e3,1e4,5e4, 2e5 ))
...
plot_concentration_surface <- function(to_show) {
  ...
  label_miRNA_count <- function(value) {
    if_else(as.numeric(value) < 1e6, paste0("#=",round(as.numeric(value)/1000),"K"),
...
TODO:
small plots:
miRNA concentration (10 to 1e5 cpc in 3T3) x target concentration relative to miRNA concetration
color: efficiency at 20hours

mark fibroblasts, oocytes on the small plots

grid - k_cat_coeff (possibly time)
