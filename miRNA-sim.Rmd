...
if(log_scale) {
  ...
  target_concentrations = seq(log(min_miRNA_concentration), log(max_miRNA_concentration * 10), length.out = N_steps) %>% exp()
} else {
  miRNA_concentrations = seq(min_miRNA_concentration, max_miRNA_concentration, length.out = N_steps)
  target_concentrations = seq(min_miRNA_concentration, max_miRNA_concentration * 10, length.out = N_steps)
}
...
k_cat_coeffs_concentration <- 1 / c(1, 2, 4, 5, 10, 20)
...
```{r plot_concentration, fig.height=6}
plot_efficiency_surface <- function(to_show, time_h, fixed_data) {
 ...
 p <- to_show %>% filter(type == "target", time_h == !!time_h) %>% 
   mutate(concentration = if_else(concentration < 0, 0, concentration),
          relative_concentration = concentration / initial_target) %>%
  ggplot(aes(x = total_enzyme, y = total_target, fill = relative_concentration, z = relative_concentration)) + 
   geom_raster() + 
   geom_vline(data = fixed_data, mapping = aes(xintercept = miRNA_concentration, color = condition)) +
   scale_fill_distiller("Target uncleaved", palette = "Spectral") +
   facet_wrap(~k_cat_coeff, labeller= labeller(k_cat_coeff = function(x) {paste0("coeff = ",x)})) +
