...
miRNA_sim_single <- function(condition, times, init_complex_equilibrium = TRUE) {
  ...
  if(init_complex_equilibrium) {
    ...
    abs_diff <- abs(check - K_D)
    if(abs_diff > 1e-6) {
      stop(paste("Not equilibrium, K_D =  ",K_D, "check = ", check, "  diff = ", abs_diff))
    }
...
```{r}
min_miRNA_cpc_3T3 =  10
max_miRNA_cpc_3T3 = 1e5
...
miRNA_concentrations = seq(log(min_miRNA_concentration), log(max_miRNA_concentration), length.out = 30) %>% exp()

target_concentrations_relative = seq(log(0.5), log(100), length.out = 30) %>% exp()

times_concentration_hours = c(15,20,25)
times_concentration = times_concentration_hours * 60 * 60 * time_unit_from_sec

if(!all(times_to_show %in% times)) {
  stop("Invalid times to show")
}

k_cat_coeffs_concentration <- 1 / c(1, 2, 4, 5, 10, 20)

all_conditions_concentration <- tibble(total_enzyme = miRNA_concentrations) %>%
  crossing(tibble(target_relative = target_concentrations_relative)) %>%
  crossing(k_cat_coeff = k_cat_coeffs_concentration) 
  
cell_info = tibble(condition = c("let7 in 3T3", "let7 in oocyte"),
                   miRNA_concentration = c(42000 / (mole_unit * volume_3T3), 42000 / (mole_unit * volume_mouse_oocyte)))
```
...
sim_results_concentration <- all_conditions_concentration %>%
  mutate(synthesis = 0,
         total_target = total_enzyme * target_relative
         ) %>%
...
plot_efficiency_surface <- function(to_show, time_h, fixed_data) {
 p <- to_show %>% filter(type == "target", time_h == time_h) %>% 
  ...
  ggplot(aes(x = total_enzyme, y = target_relative, fill = relative_concentration, z = relative_concentration)) + 
   geom_raster() + 
   geom_vline(data = fixed_data, mapping = aes(xintercept = miRNA_concentration, color = cell), inherit.aes= FALSE) +
   scale_fill_distiller(palette = "Spectral") +
   facet_wrap(~k_cat_coeff, labeller= labeller(k_cat_coeff = function(x) {paste0("coeff = ",x)})) +
   scale_x_continuous("miRNA concentration", trans = "log2") +
   scale_y_continuous("relative target concentration", trans = "log2") +
   ggtitle(paste0("Relative concentration at ", time_h,"h"))
