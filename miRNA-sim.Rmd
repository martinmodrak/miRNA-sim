---
title: "miRNA and siRNA modelling"
output: html_notebook
---


```{r setup}
library(deSolve)
library(tidyverse)
library(cowplot)
```

# Outstanding questions

- do we measure uncleave target RNA or free target mRNA? (matters only in 3T3e cells)
  We measure protein (AGO bound is repressed)
- What exactly is measured? All sequences complementary to let7?
- total RNA MSC?

# Assumptions

Using nM/l as unit, K_m and k_cat from Zamore 2015.

Notes 2018-09-05:
miRNA does not deplete in ooczt ine  20h
miRNA silencing hapens on the order of hours
Need to consider competition with siRNAs and loading unloading
AGO + siRNA will stay loaded, AGO + miRNA might not
AGO + miRNA has intermediate stages

Denzler has total miRNA to siRNA counts (hopefully). 
3T3 cell has only miRNA (but siRNA can be induced)

miRNA seed vs. seed and something


Further notes:
In Zamore 2012 Fig. 7 the $K_M$ values are about a factor of 2.5 off from what would be calculated from $k_{on}$, $k_{off}$ and $k_cat$ - those are different experiments.

Bosson 2014:
Absolute levels of miRNAs for various cell types are estimated in the range of tens to 
120,000 copies per cell

This method provided target cpc estimates of 1,200 8-mers, 5,000 7-mers, and 22,000 6-
mers on average for 3′ UTR sites of each active miRNA ESC family (Figure 3D, lines). For 
reference to other cell types, these values correspond to 0.8%, 3.2%, and 14.1% of the total 
mRNA concentration in the cell. 

Assuming AGO is not rate limiting, we simplify the reaction and treat the  specific miRNA/siRNA species as enzyme in the reactions.

```{r}
#mole_unit <- 6.022e20 #Mili mole
# mole_unit_from_nano_mole <- 1e-6
# mole_unit_from_mole <- 1e3


#Using litre as volume unit! (since constants are in moles per litre)
mole_unit <- 6.022e14 #nano mole
mole_unit_from_nano_mole <- 1
mole_unit_from_mole <- 1e9
time_unit_from_sec <- 1/3600 #Use hour instead of seconds

K_M_siRNA <- 0.1 * mole_unit_from_nano_mole
k_on_siRNA <- 3.6e7 / (mole_unit_from_mole * time_unit_from_sec)
k_off_siRNA <- 7.7e-4 / time_unit_from_sec
k_cat_siRNA <- 8.1e-4 / time_unit_from_sec

k_on_miRNA <- 0.2e8 / (mole_unit_from_mole * time_unit_from_sec)
k_off_miRNA <- 0.051e-2 / time_unit_from_sec


volume_3T3 <- 4.4e-12
volume_oocyte <- 260e-12

total_RNA_ESC <- 158000 #Bosson 2014
total_RNA_ESC <- 750000 #Bosson 2014 Fig S1
total_RNA_3T3 <- 1e6 #5e5 - 1e6
total_RNA_oocyte <- 25e6

let7_3T3 <- 5e4 / mole_unit
let7_oocyte <- 3.5e4 / mole_unit

let7_targets_MSC <- (1380+2884+20414) / mole_unit #8-mer, 7-mer and 6-mers respectively from Bosson 2014
let7_targets_3T3 <-  let7_targets_MSC
let7_targets_oocyte <- let7_targets_3T3 * (volume_oocyte / volume_3T3)

times <- seq(0,24*60*60 * time_unit_from_sec, length.out = 100)


one_step_degradation <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    d_target <- -k_on * enzyme * target + k_off * complex 
    d_enzyme <- -k_on * enzyme * target + k_off * complex + k_cat * complex
    d_complex <- k_on * enzyme * target - k_off * complex - k_cat * complex
    list(c(d_target, d_enzyme, d_complex))
  })
}

plot_ode_result <- function(out, title) {
  out_tidy <- out %>% as.data.frame() %>% 
    gather(type, concentration, -time) %>%
    mutate(time_h = time / (3600 * time_unit_from_sec)) 
    
  
  uncleaved_plot <- out_tidy %>% 
    filter(type != "enzyme") %>%
    group_by(time_h) %>%
    summarise(all_uncleaved = sum(concentration)) %>%
    (function(data) { 
      ggplot(data,aes(x = time_h, y = all_uncleaved)) + geom_line() +
        ggtitle(paste0(title," uncleaved RNAs"))  +
        ylim(-1e-5, max(data$all_uncleaved)) 
    })

  print(uncleaved_plot)
  
  auxiliary_plot <- 
    ggplot(out_tidy, aes(x = time_h, y = concentration, color = type, linetype = type)) + geom_line() +
    ggtitle(paste0(title," all reactants") ) + 
    ylim(-1e-5, max(out_tidy$concentration))


  print(auxiliary_plot)
}

```

# let7 modelling

Modulating just k_cat for let7 is enough - to get full let7 efficiency at 3T3 cells but have negligible degradation in oocyte for $k_{cat,\mathrm{let7}} \in (0.05 * k_{cat,\mathrm{siRNA}}, 0.3 * k_{cat,\mathrm{siRNA}})$.

```{r}
for(k_cat_coef in c(0.05,0.2,0.3)) {
  k_cat_let7_one_step <- k_cat_coef * k_cat_siRNA
  params_let7_one_step <- c(k_cat = k_cat_let7_one_step, k_on = k_on_miRNA, k_off = k_off_miRNA)
  
  state <- c(target = let7_targets_3T3 / volume_3T3, enzyme = let7_3T3 / volume_3T3, complex = 0)
  
  out_let7_one_step_3T3 <- ode(y = state, times = times, func = one_step_degradation, parms = params_let7_one_step)
  
  plot_ode_result(out_let7_one_step_3T3, paste0("let7 - 3T3 k_cat_coef = ", k_cat_coef))
  
  
  state <- c(target = let7_targets_oocyte / volume_oocyte, enzyme = let7_oocyte / volume_oocyte, complex = 0)
  
  out_let7_one_step_oo <- ode(y = state, times = times, func = one_step_degradation, parms = params_let7_one_step)
  
  plot_ode_result(out_let7_one_step_oo, paste0("let7 - oocyte k_cat_coef = ", k_cat_coef))
}
```

# Sanity check: simulating MM kinetics for siRNA 

We want to get complete degradation with siRNA in oocyte in a reasonable time. Assuming siRNA with the same concentration as let7

```{r}

hypothetical_siRNA_oocyte <- let7_oocyte
hypothetical_siRNA_targets_oocyte <- let7_targets_oocyte

params_siRNA <- c(k_cat = k_cat_siRNA, k_on = k_on_siRNA, k_off = k_off_siRNA)


state <- c(target = hypothetical_siRNA_targets_oocyte / volume_oocyte, enzyme = hypothetical_siRNA_oocyte / volume_oocyte, complex = 0)

out_siRNA <- ode(y = state, times = times, func = one_step_degradation, parms = params_siRNA)
plot_ode_result(out_siRNA,paste0("Hypothethical siRNA, oocyte"))
  
```

# Experimenting with two step reactions

```{r}
k_cat_let7_two_step <- 0.2 * k_cat_siRNA
k_intermediate_let7 <- 1 * k_cat_siRNA


two_step_degradation <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    d_target <- -k_on * enzyme * target + k_off * complex 
    d_enzyme <- -k_on * enzyme * target + k_off * complex + k_cat * intermediate
    d_complex <- k_on * enzyme * target - k_off * complex - k_intermediate * complex
    d_intermediate <- k_intermediate * complex - k_cat * intermediate
    list(c(d_target, d_enzyme, d_complex, d_intermediate))
  })
}

params_let7 <- c(k_on = k_on_miRNA, k_off = k_off_miRNA, k_cat = k_cat_let7_two_step, k_intermediate = k_intermediate_let7)

state_3T3 <- c(target = let7_targets_oocyte / volume_3T3, enzyme = let7_3T3 / volume_3T3, complex = 0, intermediate = 0)

out_let7 <- ode(y = state_3T3, times = times, func = two_step_degradation, parms = params_let7)



plot_ode_result(out_let7, "3T3")

state_oo <- c(target = let7_targets_oocyte / volume_oocyte, enzyme = let7_oocyte / volume_oocyte, complex = 0, intermediate = 0)

out_let7_oo <- ode(y = state_oo, times = times, func = two_step_degradation, parms = params_let7)

plot_ode_result(out_let7_oo, "Oocyte")


```

