...
```{r}
animals_levels_raw <- c("mouse","pig","cow")
animals_labels <- c("Mouse","Porcine","Bovine")
```
...
base_plot <- function(data, x, y) {
  ...
  ggplot(data, aes(x = !!x, y = !!y, color = !!x, shape = !!x)) +
    ...
    mean_summary +
    geom_point(position = position_jitter(width = 0.25), size = point_size) +
...
```{r, fig.height=3, fig.width=6}
combined_file <- here("private_data", "190612 for martin.xlsx")
...
let7a_porcine_data <- read_excel(combined_file, sheet = "Let-7a porcine 40hours", col_names = c("Animal","Reporter","Activity"))
plots$let7a_porcine <- base_plot(let7a_porcine_data, Reporter, Activity)
